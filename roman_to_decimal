# Convert a string with a roman numeral to decimal, tested to work up to 100 decimal
def roman_to_decimal(roman):
  ...
  numbers = [intervals[letter] for letter in roman]
  highests = [i for i, x in enumerate(numbers) if (x - (numbers[i+1] if (i+1 < len(numbers)) else 0) < 0)]
  result = numbers
  for high in highests:
    result[high+1] -= result[high]
  for index, high in enumerate(highests):
    del result[high-index]
...
if __name__ == "__main__":
  ...
  tests = []
  for decimal, roman in roman_numbers:
    result = roman_to_decimal(roman)
    surfix = "passed" if (result == decimal) else "failed"
    tests.append(surfix)
    print(f"{surfix}: {roman}, resulted in {result}, should be {decimal}")

  print(f"{tests.count('passed')}/{len(roman_numbers)} passed, {tests.count('failed')} failed")
